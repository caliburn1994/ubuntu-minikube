...
## install
#K8S_SERVICE="alone"
#if ! kubectl get service "${K8S_SERVICE}-redis-cluster" &>/dev/null; then
...
#
#
#export REDIS_PASSWORD=$(kubectl get secret --namespace default "${K8S_SERVICE}-redis-cluster" -o jsonpath="{.data.redis-password}" | base64 --decode)
#
#
#
#
